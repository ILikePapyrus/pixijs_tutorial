...
function createTrainHead(app) {
    ...
    app.ticker.add((time) => {
        const dx = time.deltaTime * 0.15;

        backWheel.rotation += dx * (smallWheelRadius / bigWheelRadius);
        midWheel.rotation += dx;
        frontWheel.rotation += dx;
    });
